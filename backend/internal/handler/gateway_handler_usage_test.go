...
func TestUsageQuotaLimitedIncludesSubscriptionMetadata(t *testing.T) {
	...
	require.True(t, dailyStart.Add(24*time.Hour).Equal(*response.Subscription.DailyResetAt))
	require.True(t, weeklyStart.Add(7*24*time.Hour).Equal(*response.Subscription.WeeklyResetAt))
	require.True(t, monthlyStart.Add(30*24*time.Hour).Equal(*response.Subscription.MonthlyResetAt))
	require.True(t, expiresAt.Equal(response.Subscription.ExpiresAt))
