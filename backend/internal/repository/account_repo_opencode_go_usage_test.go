...
func openCodeGoMergeMockColumns() []string {
	return []string{
		...
		"base_url_changed", "enabled", "rate_sync_enabled", "snapshot", "upstream_identity",
...
func TestLockAndMergeAccountProbeExtraPreservesOpenCodeGoManagedState(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			mock.ExpectQuery(`(?s)`+regexp.QuoteMeta("SELECT")+`.*`+regexp.QuoteMeta("FOR NO KEY UPDATE")).
				WithArgs(tt.account.ID, tt.account.Platform, tt.account.Type, string(credentials), nil, "null", "null").
				WillReturnRows(sqlmock.NewRows(openCodeGoMergeMockColumns()).
					AddRow(false, false, tt.proxyUnchanged, false, nil, nil, nil, nil, nil, nil, nil, tt.groupUnchanged, tt.databaseAuto, tt.databaseSnapshot))
...
func TestUpdateCredentialsOpenCodeGoIdentityChangeClearsManagedExtra(t *testing.T) {
	client, mock := newOllamaCloudUsageRepositoryTestClient(t)
	mock.ExpectBegin()
	mock.ExpectQuery(`(?s)SELECT COALESCE.*FOR NO KEY UPDATE`).
...
func TestBulkUpdateOpenCodeGoBaseURLClauseIsNullSafe(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)SELECT id,.*FOR NO KEY UPDATE`).
		WillReturnRows(sqlmock.NewRows([]string{"id", "base_url_changed"}).AddRow(17, false))
	mock.ExpectExec(`(?s)UPDATE accounts.*IS NOT TRUE`).
		WillReturnResult(sqlmock.NewResult(0, 1))
