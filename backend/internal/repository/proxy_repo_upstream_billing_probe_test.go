...
func TestProxyUpdateInvalidatesBoundProbeSnapshotsAndEnqueuesOutboxAtomically(t *testing.T) {
	...
	expectProxyUpdateReload(mock, 9, "new.example", "user", "pass")
		mock.ExpectQuery(`(?s)UPDATE accounts.*- 'upstream_billing_probe'.*- 'upstream_identity'.*- 'ollama_cloud_usage_snapshot'.*type = 'apikey'.*extra \? 'upstream_billing_probe'.*extra \? 'upstream_identity'.*platform IN \('openai', 'anthropic', 'kimi', 'zhipu', 'deepseek', 'minimax'\).*extra \? 'ollama_cloud_usage_snapshot'.*RETURNING id`).
		WithArgs(int64(9)).
