...
type publicConfigSnapshot struct {
	...
	invalidTokenEndpointIDs []string
}
...
func (m *ConfigManager) Reload(ctx context.Context) error {
	...
	m.public.Store(&publicConfigSnapshot{
		...
		invalidTokenEndpointIDs: append([]string(nil), active.InvalidTokenEndpointIDs()...),
	})
...
func (m *ConfigManager) Public() (PublicConfig, error) {
	...
	return PublicFromStorage(cloneStorageConfig(snapshot.storage), snapshot.riskControlEnabled, snapshot.invalidTokenEndpointIDs), nil
}
