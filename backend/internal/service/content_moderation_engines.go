...
func (cfg *ContentModerationConfig) engineProfile(engine string) *ContentModerationEngineConfig {
	...
	if engine == ContentModerationEngineTypeSafe {
		...
		p = ContentModerationEngineConfig{BaseURL: cfg.BaseURL, Model: cfg.Model, ProxyID: cfg.ProxyID, APIKeys: normalizeModerationAPIKeys(cfg.APIKeys), TimeoutMS: cfg.TimeoutMS, RetryCount: cfg.RetryCount, Thresholds: cfg.Thresholds}
	}
...
func (cfg *ContentModerationConfig) effectiveEngine(engine string) *ContentModerationConfig {
	...
	if out.Engine == ContentModerationEngineTypeSafe {
		// OpenAI endpoint credentials must never be sent to the TypeSafe engine.
		out.ModerationEndpoints = nil
	}
	return out
}
