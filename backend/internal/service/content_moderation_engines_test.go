...
func TestContentModerationTypeSafeFailureOpenAndKeyIsolation(t *testing.T) {
	...
	typeSafeTargets := cfg.moderationTargets()
	openAIConfig := cloneContentModerationConfig(cfg)
	openAIConfig.Engine = ContentModerationEngineOpenAI
	openAITargets := openAIConfig.moderationTargets()
	require.True(t, s.isAPIKeyFrozen(typeSafeTargets[0], time.Now()))
	require.False(t, s.isAPIKeyFrozen(openAITargets[0], time.Now()))
	require.Equal(t, "frozen", s.apiKeyStatuses(typeSafeTargets)[0].Status)
	require.Equal(t, "unknown", s.apiKeyStatuses(openAITargets)[0].Status)
	require.Zero(t, s.preBlockAPIKeyAvailableCount(typeSafeTargets))
}
