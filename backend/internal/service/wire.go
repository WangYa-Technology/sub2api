...
func ProvideChannelMonitorV2Aggregator(repo ChannelMonitorV2Repository, db *sql.DB, settingService *SettingService, cfg *config.Config) *ChannelMonitorV2Aggregator {
	...
	applyGlobalBackgroundTaskEligibility(&aggregator.instanceID, cfg)
	if os.Getenv("CHANNEL_MONITOR_V2_DISABLE_AGGREGATOR") == "1" || !backgroundTaskLeaderEligible(aggregator.instanceID) {
		return aggregator
	}
	aggregator.Start()
	return aggregator
