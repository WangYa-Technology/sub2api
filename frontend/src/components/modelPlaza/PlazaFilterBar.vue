  ...
  <div class="space-y-3">
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      <button
        ...
        :key="`platform-${p}`"
        type="button"
        class="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition disabled:cursor-not-allowed disabled:opacity-40 disabled:grayscale"
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      <button
        ...
        :key="`group-${g.id}`"
        type="button"
        class="rounded-lg px-3 py-1.5 text-xs font-medium transition disabled:cursor-not-allowed disabled:opacity-40 disabled:grayscale"
...
/**
 * 平台与分组互为约束(faceted):某选项可点 ⟺ 在另一维当前选择下仍有分组命中。
