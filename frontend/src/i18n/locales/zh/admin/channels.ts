export default {
    ...
    channels: {
      ...
      emptyModelsInPricing: '请为 {platform} 的每条定价规则至少添加一个模型',
      loadError: '加载渠道列表失败',
    ...
    riskControl: {
      ...
      additionalEndpoints: '其他审核端点',
      additionalEndpointsHint: '不同 Base URL 的 Key 分别配置为独立端点，所有启用端点会共同参与审核调度和故障切换。',
      addEndpoint: '添加端点',
      newEndpoint: '新审核端点',
      endpointName: '端点名称',
      endpointNamePlaceholder: '例如：备用 OpenAI',
      endpointApiKeys: '该端点的 API Keys',
      endpointApiKeysPlaceholder: '每行一个 Key；留空保留已保存的 Key',
      endpointApiKeysHint: '新输入的 Key 会追加到该端点，已保存的 Key 不会返回到浏览器。',
      endpointFieldsRequired: '请完整填写每个审核端点的名称、Base URL 和模型。',
      moderationKeyTargets: '内容审核 Key',
      moderationKeyTargetsHint: '每个 Key 绑定独立的 Base URL、模型和代理；必须测试成功后才能加入列表。',
      addModerationKey: '添加 Key',
      moderationApiKey: 'OpenAI API Key',
      keyTestRequired: '填写完整参数并完成连接测试后才能添加。',
      keyTestPassed: '测试通过，参数可加入 Key 列表',
      keyTestFailed: '审核 Key 测试失败',
      testConnection: '测试连接',
      addToKeyList: '添加到 Key 列表',
      noModerationKeys: '尚未配置内容审核 Key',
      pendingSave: '待保存',
      savedKey: '已保存',
      editModerationKey: '修改已有 Key',
      editKeyTestRequired: '修改端点参数后必须重新测试，测试通过才能应用。',
      applyKeyChanges: '应用修改',
      directConnection: '直连',
      proxyID: '代理 #{id}',
      moderationEndpoint: '审核端点',
      model: '模型名',
