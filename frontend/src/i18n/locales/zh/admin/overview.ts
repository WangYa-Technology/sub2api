export default {
    ...
    groups: {
      ...
      claudeMaxSimulation: {
        ...
        tooltip:
          '启用后，对于上游未提供缓存写入用量的 Claude 模型，系统会在保持总 token 不变的前提下，将 token 确定性映射为少量输入 token 加 1 小时缓存创建 token。',
        enabled: '已启用（模拟 1 小时缓存）',
        disabled: '已禁用',
        hint: '仅调整用量计费日志中的 token 分类，不会持久化每次请求的映射状态。'
      },
      mcpXml: {
        title: 'MCP XML 协议注入',
