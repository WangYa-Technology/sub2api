export default {
  ...
  modelPlaza: {
    ...
    table: {
      ...
      paidPrice: '实付价格(折后)',
      paidPriceHelpLabel: '查看实付价格说明',
      paidPriceHelp: '这是调用模型时的最终计费价格，已包含渠道价格、生效倍率（分组倍率或用户专属倍率）和充值换算倍率。Token 模型按每 100 万 Token 计价，按次或按图片模型以对应单位为准。',
      officialPrice: '官方价格',
      officialPriceHelpLabel: '查看官方价格说明',
      officialPriceHelp: '模型服务商公开的官方参考价格，以美元计价且不乘本站的生效倍率或充值换算倍率。该价格仅用于对比，不是本站的实际扣费价格。',
      rate: '折扣倍率',
      rateHelpLabel: '查看折扣倍率说明',
      rateHelp: '仅比较输出价格：折扣倍率 =（实付输出价格（人民币）÷ 6.8）÷ 官方输出价格（美元）。≈ 0.04 倍表示实付输出价格约为官方价格的 4%。此倍率仅供参考，实际扣费以实付价格为准。',
      paidUnitPerMillion: '¥ / 1M token',
