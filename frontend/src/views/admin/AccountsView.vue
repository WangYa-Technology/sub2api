...
import type { Account, AccountListItem, AccountPlatform, AccountSchedulerGroupScore, AccountType, AccountUsageInfo, Proxy as AccountProxy, AdminGroup, WindowStats, ClaudeModel, UpstreamBillingProbeSnapshot, UpstreamIdentitySnapshot, UpstreamQuotaQueryResult } from '@/types'
...
const readPersistedUpstreamQuota = (account: Account) => {
  ...
  try {
    ...
    try { window.localStorage.removeItem(key) } catch { /* localStorage may be disabled. */ }
    return null
  }
}
const persistUpstreamQuota = (account: Account, result: UpstreamQuotaQueryResult) => {
  ...
  try {
    window.localStorage.setItem(key, JSON.stringify({ identity: upstreamQuotaCacheIdentity(account), result }))
  } catch {
...
const handleBulkDelete = async () => {
  ...
  try {
    ...
    const deletedAccountIds = accountIds.filter(id => !failedIds.has(id))
    ...
    if (result.failed > 0) {
      ...
      setSelectedIds(failedIds.size ? [...failedIds] : accountIds)
    } else {
