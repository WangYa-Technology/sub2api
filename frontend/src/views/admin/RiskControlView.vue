        ...
        <div class="space-y-6">
          ...
          <div v-if="activeSettingsTab === 'basic'" class="space-y-5 pb-24">
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
              ...
              <div v-if="configForm.engine === 'typesafe'">
            ...
            <section v-if="configForm.engine === 'openai'" data-test="audit-openai-keys" class="mb-20 overflow-hidden rounded-lg border border-gray-200 bg-white dark:border-dark-700 dark:bg-dark-800">
            ...
            <div v-if="configForm.engine === 'typesafe'" class="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm dark:border-dark-700 dark:bg-dark-800">
                ...
                <div data-test="audit-key-statuses" class="rounded-lg border border-gray-100 bg-gray-50 p-3 dark:border-dark-700 dark:bg-dark-900/30">
                  ...
                  <div v-if="moderationTestResult" class="mt-4 rounded-lg border border-gray-100 bg-white p-3 dark:border-dark-700 dark:bg-dark-800">
                    <p v-if="moderationTestResult?.engine_meta" class="mb-2 break-words text-xs text-gray-500">
                      {{ engineLabel(moderationTestResult?.engine_meta?.engine ?? configForm.engine) }} · {{ moderationTestResult?.engine_meta?.model }} · {{ moderationTestResult?.engine_meta?.rules_version }}
...
function applyConfig(config: ContentModerationConfig) {
  ...
  const openAIConfig = config.engine_configs?.openai ?? config
  const configuredEndpoints = Array.isArray(openAIConfig.moderation_endpoints) ? openAIConfig.moderation_endpoints : []
  ...
  if (defaultEndpoint) {
    Object.assign(engineDrafts.value.openai!, {
      base_url: defaultEndpoint.base_url,
      model: defaultEndpoint.model,
      proxy_id: defaultEndpoint.proxy_id ?? null,
      api_key_count: defaultEndpoint.api_key_count,
      api_key_configured: defaultEndpoint.api_key_count > 0,
      api_key_masks: [...(defaultEndpoint.api_key_masks ?? [])],
      api_key_statuses: [...(defaultEndpoint.api_key_statuses ?? [])],
    })
  }
...
async function loadAll() {
  ...
  try {
    ...
    if ((runtimeStatus.engine ?? 'openai') === configForm.engine && Array.isArray(runtimeStatus.api_key_statuses) && runtimeStatus.api_key_statuses.length > 0) {
...
async function saveConfig() {
  ...
  try {
    ...
    engineDrafts.value[configForm.engine] = captureEngineDraft()
    const openAIDraft = engineDrafts.value.openai!
    const openAIKeys = parseApiKeys(openAIDraft.api_keys_text)
    const defaultEndpointPayload = {
      id: 'default',
      name: 'OpenAI',
      base_url: openAIDraft.base_url,
      model: openAIDraft.model,
      proxy_id: openAIDraft.proxy_id ?? 0,
      enabled: true,
      ...(openAIDraft.clear_api_key || openAIDraft.api_keys_mode === 'replace' ? { api_keys: openAIDraft.clear_api_key ? [] : openAIKeys } : {}),
      ...(!openAIDraft.clear_api_key && openAIDraft.api_keys_mode === 'append' && openAIKeys.length > 0 ? { add_api_keys: openAIKeys } : {}),
      ...(!openAIDraft.clear_api_key && openAIDraft.api_keys_mode === 'append' && openAIDraft.pendingDeletes.length > 0 ? { delete_api_key_hashes: [...openAIDraft.pendingDeletes] } : {}),
    }
