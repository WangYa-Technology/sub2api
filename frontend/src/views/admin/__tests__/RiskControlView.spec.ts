...
describe('admin RiskControlView', () => {
  ...
  it.each(['openai', 'typesafe'] as const)('shows the selected draft engine keys while %s remains active', async (activeEngine) => {
    ...
    const keyPanel = () => wrapper.find('[data-test="audit-openai-keys"]').exists()
      ? wrapper.get('[data-test="audit-openai-keys"]')
      : wrapper.get('[data-test="audit-key-statuses"]')
    ...
    expect(keyPanel().text()).toContain(otherEngine === 'openai' ? 'admin.riskControl.savedKey' : 'admin.riskControl.apiKeyStatusOk')
  ...
  it('preserves both engine drafts and saves thresholds independently', async () => {
    ...
    await flushPromises()
    await findButtonByText(wrapper, 'admin.riskControl.openSettings').trigger('click')
    // HCAI edits OpenAI URLs per tested key, not through the TypeSafe flat URL field.
    ...
    expect(updateConfig).toHaveBeenCalledWith(expect.objectContaining({ engine: 'openai', engine_configs: {
      openai: expect.objectContaining({ base_url: baseConfig().base_url, thresholds: expect.objectContaining({ sexual: 0.73 }) }),
