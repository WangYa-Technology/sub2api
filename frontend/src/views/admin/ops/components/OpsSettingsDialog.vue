...
function buildWeComUpdate(): WeComNotificationConfigUpdate | null {
  ...
  const payload: WeComNotificationConfigUpdate = {
    ...
    account_error_enabled: weComConfig.value.account_error_enabled,
    proxy_expiry_enabled: weComConfig.value.proxy_expiry_enabled,
...
async function saveAllSettings() {
  ...
  try {
    ...
    const weComUpdate = buildWeComUpdate()
    await Promise.all([
      ...
      weComUpdate ? opsAPI.updateWeComNotificationConfig(weComUpdate) : Promise.resolve(),
    ...
    <div v-else-if="runtimeSettings && emailConfig && weComConfig && advancedSettings" class="space-y-6">
      ...
      <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-700/50">
        ...
        <div class="space-y-4">
          ...
          <div v-if="emailConfig.alert.enabled" class="space-y-3 border-t border-gray-200 pt-4 dark:border-dark-600">
            <div class="flex items-center justify-between gap-4">
              <div>
                ...
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.ops.settings.accountErrorNotificationHint') }}
                </p>
              </div>
              <Toggle v-model="emailConfig.alert.account_error_enabled" />
            </div>
            ...
            <div class="flex items-center justify-between gap-4">
              <div>
                ...
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.ops.settings.proxyExpiryNotificationHint') }}
                </p>
              </div>
              <Toggle v-model="emailConfig.alert.proxy_expiry_enabled" />
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="space-y-4">
          <div class="flex items-center justify-between gap-4">
            <div>
              ...
              <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('admin.ops.settings.wecom.enabledHint') }}</p>
            </div>
          ...
          <div>
            ...
            />
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('admin.ops.settings.wecom.webhookHint') }}</p>
          </div>
          ...
          <div class="space-y-3 border-t border-gray-200 pt-4 dark:border-dark-600">
            <div class="flex items-center justify-between gap-4">
              <div>
                <div class="text-sm font-medium text-gray-900 dark:text-white">{{ t('admin.ops.settings.wecom.accountError') }}</div>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('admin.ops.settings.wecom.accountErrorHint') }}</p>
              </div>
              <Toggle v-model="weComConfig.account_error_enabled" />
            </div>

            <div class="flex items-center justify-between gap-4">
              <div>
                <div class="text-sm font-medium text-gray-900 dark:text-white">{{ t('admin.ops.settings.wecom.proxyExpiry') }}</div>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('admin.ops.settings.wecom.proxyExpiryHint') }}</p>
              </div>
              <Toggle v-model="weComConfig.proxy_expiry_enabled" />
            </div>

            <div class="flex items-center justify-between gap-4">
              <div>
                <div class="text-sm font-medium text-gray-900 dark:text-white">{{ t('admin.ops.settings.wecom.includeResolved') }}</div>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('admin.ops.settings.wecom.includeResolvedHint') }}</p>
              </div>
          ...
          <div class="flex justify-end">
            <button class="btn btn-secondary" type="button" :disabled="testingWeCom || !validation.valid" @click="testWeComNotification">
