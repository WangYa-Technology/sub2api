            ...
            <template #after-search>
              ...
              >
                <div class="flex max-w-full gap-1.5 overflow-x-auto">
                  ...
                  >
                    <PlatformIcon v-if="platform !== 'all'" :platform="platform" size="xs" />
...
const GROUP_PLATFORM_ORDER: GroupPlatform[] = [
  'openai', 'anthropic', 'gemini', 'grok', 'antigravity', 'composite',
...
const groupPlatformFilters = computed<GroupPlatformFilter[]>(() => {
  const availablePlatforms = new Set(formGroupOptions.value.map((option) => option.platform))
  return ['all', ...GROUP_PLATFORM_ORDER.filter((platform) => availablePlatforms.has(platform))]
})
